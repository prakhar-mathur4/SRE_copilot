...
# ---------------------------------------------------------------------------
# MetricsQL / PromQL query catalog — keyed by alert-name keyword category.
# VictoriaMetrics is PromQL-compatible, so these expressions run unchanged;
# kept standalone (not shared with PrometheusProvider) so VM-specific
# MetricsQL extensions can be added here without touching Prometheus.
# ---------------------------------------------------------------------------
...
class VictoriaMetricsProvider(DiagnosticProvider):
    """
    Diagnostic provider for VictoriaMetrics (single-node or vmselect).

    VictoriaMetrics exposes a Prometheus-compatible query API
    (``/api/v1/query`` and ``/api/v1/query_range``), so PromQL runs unchanged.
    It differs from Prometheus on its health endpoint: VM serves ``/health``
    ...
    async def collect_diagnostics(self, context: str, alert_labels: Dict[str, str]) -> str:
        """Query VictoriaMetrics for instance-specific metrics relevant to this alert.
    ...
    async def _run_query(self, client: httpx.AsyncClient, instance: str, base_query: str) -> Tuple[List[Dict], str]:
        """Run a query scoped to a specific instance. Returns (results, actual_query)."""
    ...
    def _format_value(display_name: str, raw: Any) -> str:
        """Format a raw scalar for display."""
        try:
            num = float(raw)
    ...
    async def get_time_series(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"VictoriaMetrics time series query failed: {e}")
